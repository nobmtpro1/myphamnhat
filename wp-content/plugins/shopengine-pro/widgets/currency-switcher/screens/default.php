...
?>
	<div class="shopengine-currency-switcher">
		...
		<select onchange="shopengine_currency_switcher(this.value)" class="shopengine-currency-switcher--select">
			...
			?>
					<option <?php esc_attr($currency['code'] === $session_currency_code ? 'selected' : ""); ?> value="<?php echo esc_attr($currency['code']) ?>"><?php echo esc_html($symbol . ' ' . shopengine_pro_translator('currency-switcher__currencies__name__' . $currency['_uid'], $currency['name'])) ?></option>
			<?php endif;
			endforeach; ?>
		</select>
	</div>
